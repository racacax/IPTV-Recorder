...
class RecordingProcess(subprocess.Popen):
    termination_string: Optional[str]


logging.root.setLevel(logging.NOTSET)
RUNNING_IDS: dict[int, Optional[RecordingProcess]] = {}
...
def launch_command(command_str: str, recording: Recording, termination_string: str):
    with open(recording.writing_directory + "logs.txt", "a") as f:
        ...
        command = subprocess.Popen("exec " + command_str, stdin=subprocess.PIPE, stderr=f, shell=True)
...
def start_recording(recording: Recording):
    logging.info(f"Starting recording {recording}")
    ...
    if not path.exists(recording.writing_directory):
        os.makedirs(recording.writing_directory, exist_ok=True)
    ...
    if not recording_method:
        ...
        return
    name = (
        recording.writing_directory
        + slugify(recording.name + " - " + recording.last_retry.strftime("%Y-%m-%d " "%H:%M:%S"))
        + ".mp4"
    )
    ...
    threading.Thread(
        ...
        args=[command, recording, recording_method.termination_string],
    ).start()
...
def stop_recording(recording: Recording):
    logging.info(f"Stopping recording {recording}")
    try:
        process = RUNNING_IDS[recording.id]
        if recording.id in RUNNING_IDS:
            del RUNNING_IDS[recording.id]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        while True:
            ...
            for recording in (
                Recording.objects.exclude(id__in=RUNNING_IDS)
                .filter(start_time__lte=current_date, end_time__gt=current_date)
                .prefetch_related("video_sources", "video_sources__recording_method")
                .select_related("selected_source__recording_method")
            ):
                try:
